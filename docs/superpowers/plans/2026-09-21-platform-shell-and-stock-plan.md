...
- [x] Add a client-side router to `dashboard/` (React Router) — today
      the dashboard has no routing, single view only.
- [x] Charting: skipped adding a dependency — one line/area series
      didn't justify it, built as a small hand-rolled SVG component
      (`dashboard/src/features/StockChart.tsx`) instead.
- [x] Confirm both new deps build and the existing dashboard still
      runs unchanged before touching any design work.

## Phase 1 — Visual identity reset (tokens, both apps)

- [x] Rewrite `DESIGN.md` and `.impeccable/design.json` for the new
...
- [x] Update `.impeccable/surfaces/*` direction contract(s) to match.
- [x] `dashboard/src/index.css`: `--color-accent` → emerald ramp,
...
- [x] `app/src/index.css`: same token changes, mirrored — no
      structural/layout changes to `app/`.
- [x] `dashboard/src/App.css` and `app/src/App.css`: update any
...
- [x] Manual test: existing dashboard (totals band, entries table,
...
- [x] Update `PRODUCT.md`: Positioning section reframed around the
...
- [x] `impeccable detect` mechanical scan on the new/changed surfaces
      (shell, Stock page, re-tokenized login/dashboard/app screens).
- [x] Finish-review pass (screenshots of shell nav, Stock page,
      disabled-module state, re-skinned app/ screens) — same rigor as
      the earlier Cooperative Ledger redesign.
- [x] Fix material findings in one batch; at most one more
      verification round; stop once clean.
- [x] Re-run `impeccable document` (or equivalent) so DESIGN.md and
