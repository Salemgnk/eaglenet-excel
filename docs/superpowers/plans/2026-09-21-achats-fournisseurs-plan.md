...
- [ ] Migration: `suppliers`, `purchases`, `purchases_history`,
      `supplier_payments` — same shape as clients/sales/sales_history/
      payments, RLS mirrored, `log_purchases_changes` audit trigger.
- [ ] Realtime on `purchases` and `supplier_payments`.
- [ ] Push via `supabase db push`, verify with `migration list`.

## Phase 1 — App: offline purchases + supplier cache

- [ ] `app/src/lib/suppliersCache.ts` (mirrors `clientsCache.ts`).
- [ ] `app/src/lib/purchaseDrafts.ts` (mirrors `salesDrafts.ts`).
- [ ] `app/src/lib/sync.ts`: `syncPendingPurchases` (mirrors
      `syncPendingSales`, suppliers before purchases for the same FK
      reason).
- [ ] `app/src/features/PurchaseForm.tsx` (mirrors `SaleForm.tsx`).
- [ ] Fourth tab "Achat" wired into `App.tsx`.
- [ ] Manual test: offline purchase with a new supplier syncs
      correctly in order.

## Phase 2 — Dashboard: Achats + Fournisseurs modules

- [ ] `dashboard/src/lib/useSuppliers.ts`, `usePurchases.ts`,
      `useSupplierPayments.ts` (thin `useLiveTable` wrappers).
- [ ] `dashboard/src/features/Fournisseurs.tsx` (mirrors `Clients.tsx`).
- [ ] `dashboard/src/features/Achats.tsx` (mirrors `Ventes.tsx`,
      including the owner-side "+ Nouvel achat" form).
- [ ] `Shell.tsx`: Achats and Fournisseurs become active routes.
- [ ] Manual test: owner records a purchase directly; a supplier
...
- [ ] Confirm Stock is unaffected by purchases (no query touches it).
