...
- Stock is currently an estimate: it sums own-production entries only (service-milling entries are correctly excluded, since that rice was never the mill's), with no outflow tracking yet (no sales/waste/adjustment deductions) until the Ventes module ships. Entries recorded before `entry_type` existed have no type and are excluded from the Stock total, with a count shown to the owner rather than silently dropped.
