...
- A deliberate RLS departure from `entries`: the owner can insert directly into `sales` (and `clients`), since sales can originate from the dashboard. `entries` itself is unchanged — the owner still cannot write to it.
- Planned, not yet built: Achats/fournisseurs, Employés/paie — visible in the dashboard's navigation as disabled placeholders so the owner sees the platform's intended shape, each to be scoped and built as its own sub-project. The Excel push worker is also not yet built.
