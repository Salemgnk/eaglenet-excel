...
Development uses dummy Supabase accounts (operator1, operator2, owner) for testing only — but as of 2026-09-23 the production tables (`entries`, `expenses`) hold real historical data: the owner's actual bookkeeping (an Airtable "Petty Cash Log" export, Aug 2023 – Feb 2025 for production, through Sept 2025 for expenses; the raw file lives locally at `old_data/`, gitignored, never committed) was parsed and imported — 134 production entries (2,715 bags, GH₵40,523 revenue) and 210 expenses (GH₵149,477, of which ~GH₵15,000 is genuinely unlabeled in the source and landed in category `other`). Both were attributed to the existing `owner` test profile for now (no real login exists yet — a separate, later decision). Rows from Feb 2025 onward that recorded bags but no revenue figure were deliberately left out rather than imported as GH₵0 (would have understated income). A second business line visible in that data, a rice-farming operation the owner also runs, was excluded entirely, as were personal/family expenses bundled into the same ledger. The categorization (fixed keyword rules mapping free text to the 6 expense categories) is a one-time best-effort pass, not a reusable pipeline — there's no ongoing sync with the old spreadsheet.
